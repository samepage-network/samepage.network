...
const AdminExtensionsPage = () => {
  ...
  if ("href" in data) {
    return <div>Remove query params from the url</div>;
  }
  return (
    <div className="grid grid-cols-4 gap-4">
      {APPS.slice(1).map((app) => (
        ...
        >
          ...
          {(data.versions[app.name.toLowerCase()] || []).map(
            ({ version, href }) => (
              <a
                className={
                ...
                href={href}
                download={`${app.name.toLowerCase()}-samepage.zip`}
                key={version}
              >
                <span>Download</span>
...
export const loader: LoaderFunction = (args) => {
  return remixAdminLoader(args, () => listExtensionsMetadata(args));
};
